"""
ABB SDK - Interactive Example Launcher
=======================================
...
# The banner and the menus use box drawing characters and emoji, which a Windows
# console set on a legacy code page cannot print
try:
...
CATEGORY_DESCRIPTIONS = {
    ...
    "controller": "Controller - identity, clock, options, backups",
    "system":     "System - RobotWare version, options, products, energy",
    "panel":      "Control panel - controller state, operation mode, speed ratio",
    "io":         "I/O system - networks, devices, read and write signals",
    "rapid":      "RAPID - tasks, modules, symbols, program execution",
    "motion":     "Motion system - mechanical units, positions, kinematics",
    "file":       "File system - browse, download and upload files",
    "elog":       "Event log - read and filter controller messages",
    "mastership": "Mastership - request and release the write access",
}
...
def render_category_menu(categories):
    ...
    for idx, cat_name in enumerate(cat_list, 1):
        ...
        label = f"  {icon}  {idx:>2}. {cat_name.upper():<12} ({count} example{'s' if count != 1 else ''})"
        box_line(label)
